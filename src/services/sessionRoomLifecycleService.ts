import { ObjectId, type Filter, type Document, type ClientSession } from 'mongodb';
...
/** Performs revocation inside a caller-owned transaction; notify only after that transaction commits. */
export const revokeSessionsInTransaction = async (
    filter: Filter<Document>, kind: 'session' | 'logoutAll' | 'otherSessions',
    session: ClientSession, accountId?: string, preservedSessionId?: string
): Promise<{ acknowledged: boolean; matchedCount: number; modifiedCount: number }> => {
    if (!session.inTransaction()) throw new Error('Session cleanup requires an active transaction.');
    const db = getDb()!;
    ...
    if (kind !== 'logoutAll' && !existing) return result;
    const userId = accountId ?? existing?.userId;
    if (!userId || typeof userId !== 'string' || !/^[a-f0-9]{24}$/.test(userId)) return result;
    const now = new Date();
...
export const revokeSessionsWithRoomCleanup = async (
    ...
    try {
        await session.withTransaction(async () => {
            result = await revokeSessionsInTransaction(filter, kind, session, accountId, preservedSessionId);
